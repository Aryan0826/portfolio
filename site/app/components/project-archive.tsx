  ...
  return (
    <article className={`project-card accent-${project.accent}`}>
      ...
      <div className="project-card-body">
        ...
        <div className="learning-delta" aria-label="Learning progression">
          <div><small>STARTED WITH</small><span>{project.startedWith}</span></div>
          <i aria-hidden="true">-&gt;</i>
          <div><small>LEFT WITH</small><span>{project.learned}</span></div>
        </div>

        <details>
          <summary><span>Open build notes</span><b aria-hidden="true">+</b></summary>
          <dl className="project-detail-grid">
            <div><dt>WHAT I BUILT</dt><dd>{project.built}</dd></div>
            <div><dt>WHY</dt><dd>{project.reason}</dd></div>
            <div><dt>HARDEST PART</dt><dd>{project.challenge}</dd></div>
      ...
      <button className="card-terminal-link" onClick={() => onOpenTerminal(`cat ${project.slug}`)}>
        <span>&gt;_ cat {project.slug}</span><span>CLI</span>
      </button>
    </article>
  );
}
...
export function ProjectArchive({ onOpenTerminal }: ProjectArchiveProps) {
  ...
  const filteredProjects = activeCategory === 'All'
    ? archiveProjects
    : archiveProjects.filter((project) => project.category === activeCategory as ProjectCategory);
    ...
    <div className="project-archive">
      ...
      <div className="project-filterbar" aria-label="Filter projects">
        {projectCategories.map((category) => {
          const count = category === 'All'
            ...
            : archiveProjects.filter((project) => project.category === category).length;
          ...
          return (
            ...
            >
              <span>{category}</span><small>{String(count).padStart(2, '0')}</small>
            </button>
          );
        })}
      </div>
      ...
      <div className="project-grid">
        {visibleProjects.map((project) => (
          <ProjectCard key={project.slug} project={project} onOpenTerminal={onOpenTerminal} />
        ))}
      </div>

      {activeCategory === 'All' && filteredProjects.length > 6 && (
        <button className="show-all-projects" onClick={() => setShowAll((current) => !current)}>
          <span>{showAll ? 'Show the concise edit' : `Show all ${filteredProjects.length} archive projects`}</span>
